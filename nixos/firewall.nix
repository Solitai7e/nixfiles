{config, lib, ...}:
let inherit (lib) mkIf mkDefault optionalString  mapAttrsToList mkOption
                  concatLines concatMapAttrs pipe toSentenceCase mapAttrs;
...
in {
  ...
  config = {
    ...
    networking.firewall' = pipe tables [(mapAttrs (name: {chains ? {}, ...}: {
      chains = mapAttrs (name: rules: mkDefault rules) chains;
    }))];
    boot.kernel.sysctl = {
