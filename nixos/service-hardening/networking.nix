{lib, pkgs, config, ...}:
let config' = config.systemd.hardening'.networking;
    inherit (lib) mkOption getExe getAttr readFile mapAttrsToList
                  pipe filterAttrs mapAttrs nameValuePair genAttrs';
...
in {
  ...
  config =
    let subconfigs' = pipe config.systemd.services [
          ...
          (mapAttrs (name: service: service.hardening'.networking))
        ];
          ...
          def shlex_vars(dict):
            ...
            for name, value in dict.items():
              if isinstance(value, list):
                ...
                expr += f"{name}={"" if value == None else shlex.quote(str(value))}\n"
            return expr
        ...
        '';
        configDir = runCommand "network-isolation-configs" {} ''
    ...
    in {
      systemd.services = {
        "network-isolation@" = {
          description = "Network Isolation for %i.service";
          bindsTo = ["network-isolation-bridge.service"];
          after = ["network-isolation-bridge.service"];
        ...
        network-isolation-dns = {
          description = "DNS Forwarder for Network-Isolated Services";
          partOf = ["network-isolation-bridge.service"];
          wantedBy = ["network-isolation-bridge.service"];
          after = ["network-isolation-bridge.service"];
          unitConfig.StopWhenUnneeded = true;
          serviceConfig = {
            Type = "forking";
      ...
      networking.nftables.tables.network-isolation = {
        family = "bridge";
        content = ''
          chain forward {
      ...
      systemd.packages = pipe subconfigs' [(mapAttrsToList (service: subconfig':
        let dropInFile = "lib/systemd/system/${service}.service.d/" +
                         "network-isolation.conf";
        in writeTextDir dropInFile (getAttr subconfig'.mode {
          disconnected = ''
            [Service]
            PrivateNetwork=yes
          '';
          isolated = ''
            [Unit]
            BindsTo=network-isolation@%N.service
            After=network-isolation@%N.service
