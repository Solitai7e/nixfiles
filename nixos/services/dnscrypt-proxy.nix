{lib, pkgs, config, ...}:
let config' = config.services.dnscrypt-proxy';
    inherit (lib) mkOption mkEnableOption mkPackageOption getExe
...
in {
  ...
  config = mkIf config'.enable {
    ...
    systemd.services.dnscrypt-proxy =
      let configFile = writeTOML "dnscrypt-proxy.toml" config'.settings; in {
        ...
        serviceConfig = {
          ...
          SocketBindDeny = "any";
          SocketBindAllow = "53";
          BindPaths = ["/data/state/dnscrypt-proxy:/var/lib/dnscrypt-proxy"];
          SystemCallFilter = [
            "@basic-io" "@file-system" "@sync" "@io-event"
            "@network-io" "@signal" "@process"
          ];
        };
        hardening' = {
          enable = true;
    ...
    users.users.dnscrypt-proxy = {
      isSystemUser = true;
      group = "dnscrypt-proxy";
      home = "/data/state/dnscrypt-proxy";
    };
