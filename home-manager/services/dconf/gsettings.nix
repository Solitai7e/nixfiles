  ...
  config.dconf.settings =
    ...
    in mkMerge (pipe config'.mappings [(concatMap (mapping:
      let schemas = if mapping.schemaId != null
            then [(schemaById mapping.schemaId // { "@path" = mapping.to; })]
            else schemasByPath mapping.to;
          suboptions = attrByPath mapping.from {} options;
            ...
            settings = mergeAttrsList (forEach keys (key:
              let name = key."@name";
                  ...
                  subconfigPath = pipe "${path}/${name}" [
                    (strings.removePrefix "${mapping.to}/")
                    pathComponents
                    (map toCamelCase)
                  ];
