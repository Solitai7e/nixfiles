{lib, pkgs, config, ...}:
let inherit (lib) mkDefault getExe pipe range concatMapAttrs;
    inherit (config.lib.file) mkOutOfStoreSymlink;
    inherit (pkgs) runCommandWith;
...
in {
  ...
  xdg.dataFile."systemd/user/xfce4-notifyd.service.d/10-after-graphical.conf".text = ''
    [Unit]
  ...
  systemd.user.services.xfce4-panel = {
    ...
    Service.ExecStart = ["${getExe pkgs.xfce4-panel} -d"];
  };
  xfconf.settings.xfce4-panel = {
    ...
    "plugins/plugin-16/mixer-command" = getExe pkgs.pavucontrol;
  ...
  xdg.configFile =
    let launchers = {
          ...
          "7" = "org.flameshot.Flameshot.desktop";
        };
