{lib, pkgs, config, ...}:
let config' = config.programs.cudatext;
    inherit (lib) mkDefault mkOption mkIf hiPrio forEach toJSON strings
                  escapeShellArg mapAttrsToList mergeAttrsList pipe;
    inherit (pkgs) runCommandLocal symlinkJoin;
    writeLexer = src: runCommandLocal "cudatext-lexer" {} ''
    ...
    '';
    writePlugin = src: runCommandLocal "cudatext-plugin" {} ''
      src=${escapeShellArg "${src}"}
      dest="$out/share/cudatext"
